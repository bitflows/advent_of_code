...
int main(int argc, char * argv[]){
	...
	uint32_t minutes_hist[60] = {0};
	for (int i = 0; i < minutes; i+=60){
		for (int j = 0; j < 60; j++){
			if (sheet[guard_max_sleep_id][i+j] == 2){
				minutes_hist[j]++;
			}
		}
	}
	...
	for (int i = 0; i < 60; i++){
		if (minutes_hist[i] > sleepiest_count){
			sleepiest_count = minutes_hist[i];
			sleepiest_minute = i;
		}
	}

	printf("Sleepiest minute is %u (%u times)\nAnswer: %u\n", sleepiest_minute, sleepiest_count, (guard_max_sleep_id * sleepiest_minute));
